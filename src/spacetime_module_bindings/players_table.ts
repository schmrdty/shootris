 ...
 */
export class PlayersTableHandle<TableName extends string> implements __TableHandle<TableName> {
  ...
  /**
   * Access to the `wallet` unique index on the table `players`,
   * which allows point queries on the field of the same name
   * via the [`PlayersWalletUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.players.wallet().find(...)`.
   *
   * Get a handle on the `wallet` unique index on the table `players`.
   */
  wallet = {
