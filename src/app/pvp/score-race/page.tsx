...
export default function ScoreRacePage() {
  ...
  useEffect(() => {
    ...
    for (const match of connection.db.pvpMatches.iter()) {
      if (match.matchType.tag === 'ScoreRaceTimeTrial') {
  ...
  const joinQueue = useCallback(() => {
    ...
    withEntryFee(() => {
      setInQueue(true);
      connection.reducers.joinMatchQueue(address.toLowerCase(), MatchType.ScoreRaceTimeTrial);
    });
  }, [connection, address, withEntryFee]);
  ...
  if (!address) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-950 via-black to-blue-950 pt-16">
        <Card className="p-8 bg-black/80 border-yellow-500">
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-black to-blue-950 px-4 py-8 pt-16">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold neon-yellow">
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-sm text-gray-400">
                        Match #{match.matchId.toString()} • {match.matchDurationSeconds}s
                      </p>
                      ...
                      {match.status.tag === 'Completed' && (
                        <p className="text-xs text-gray-500 mt-1">
                          Score: {match.player1Score.toString()} vs {match.player2Score.toString()}
                        </p>
                      )}
                    </div>
                    <div className="flex gap-2">
        ...
        <Card className="bg-black/80 border-gray-700 p-6">
          ...
          <div className="text-gray-300 space-y-2">
            <p className="flex items-center gap-2"><Timer className="h-4 w-4 text-yellow-400 shrink-0" aria-hidden="true" /> Both players race for 180 seconds (3 minutes)</p>
            <p className="flex items-center gap-2"><Star className="h-4 w-4 text-yellow-400 shrink-0" aria-hidden="true" /> Clear as many lines as possible to maximize your score</p>
            <p className="flex items-center gap-2"><Flag className="h-4 w-4 text-yellow-400 shrink-0" aria-hidden="true" /> When time runs out, highest score wins</p>
            <p className="flex items-center gap-2"><Skull className="h-4 w-4 text-red-400 shrink-0" aria-hidden="true" /> If you top out before time expires, your final score is recorded</p>
            <p className="flex items-center gap-2"><Target className="h-4 w-4 text-yellow-400 shrink-0" aria-hidden="true" /> Combos and multi-line clears give bonus points!</p>
          </div>
        </Card>
      </div>
